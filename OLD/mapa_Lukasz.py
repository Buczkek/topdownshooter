...
def from_up(mapa, col_nr):
    free_space = 0
    great_fields = []
    rows = len(mapa)
    changed = 0
    for row in range(1, rows-2):
        if mapa[row][col_nr-1] == mapa[row][col_nr+1] == 1 and free_space >= 1:
    ...
    for test in great_fields:
        ...
        test_map[test[0]][test[1]] = 1
        if flood_fill(test_map, test[0] + 1, test[1], test[0] + 1, test[1]) == True:
            test_map = deepcopy(mapa)
            test_map[test[0]][test[1]] = 1
            if flood_fill(test_map, test[0] - 1, test[1], test[0] - 1, test[1]) == True:
                los = randint(1, 10)
                if los > 2:
                    mapa[test[0]][test[1]] = 1
                    changed += 1
    if changed == 0:
        print(0)
    return mapa

def from_down(mapa, col_nr):
    free_space = 0
    great_fields = []
    rows = len(mapa)
    changed = 0
...
def build_partition_walls(mapa):
    rows = len(mapa)
    columns = len(mapa[0])
    great = []
...
def move(mapa, direction, history, mode):
    if mode == "menu":
        if direction == "N":
            if mapa != 1:
                mapa -= 1
...
print_map(world, "▓", "░", "☺", "x", "#")
print()
world = create_empty_map(16, 16, 2) # Mapa o wymiarach 16x16 szerokosc miedzy scianami 2
world = build_bearing_walls(world, 2)
print_map(world, "▓", "░", "☺", "x", "#")
print()
world = create_empty_map(17, 17, 3) # Mapa o wymiarach 17x17 szerokosc miedzy scianami 3
