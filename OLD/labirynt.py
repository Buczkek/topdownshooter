...
def doors(mapa, sb_walls, door_sizes):
    rows = len(mapa)
    columns = len(mapa[0])
    number_doors = int((rows-2)*0.25)
    doors = []
    last = []
    for column in range(sb_walls + 1, columns-2, sb_walls + 1):
        ...
        while row <= rows-2:
            if row not in last:
                actual_door_size = door_sizes[1 - randint(1, len(door_sizes))]
                for i in range(actual_door_size):
                    doors.append([row, column])
                    if row != rows-1:
                        doors.append([row + i, column])
                actual.append(row)
                row += randint(actual_door_size + 1, actual_door_size + 4)
            else:
                row += randint(1, actual_door_size +  3)
        last = actual
    return doors

def repair_walls(mapa, sb_walls):
    columns = len(mapa[0])
    for i in range(sb_walls + 1, columns):
        if mapa[0][i] == 0:
            mapa[0][i] = 1
...
def build_bearing_walls(mapa, sb_walls, door_sizes):
    door = doors(mapa, sb_walls, door_sizes)
    rows = len(mapa)
    columns = len(mapa[0])
    for column in range(sb_walls + 1, columns-2, sb_walls + 1):
        for row in range(1, rows):
            if [row, column] not in door:
                ...
                mapa[row][column] = 0
    mapa = repair_walls(mapa, sb_walls)
    return mapa
...
def build_partition_walls(mapa, sb_walls):
    ...
    for column in range(1, columns, sb_walls + 1):
        ...
        test_sb_walls = deepcopy(sb_walls)
        while column + test_sb_walls > columns-1:
            test_sb_walls -= 1
            
        if fate == 0:
            mapa = from_up(mapa, column, test_sb_walls)
        else:
            mapa = from_down(mapa, column, test_sb_walls)
    return mapa
...
def create_world(m, n, sb_walls, doors_sizes):
    mapa = create_empty_map(m, n, sb_walls)
    mapa = build_bearing_walls(mapa, sb_walls, doors_sizes)
    mapa = build_partition_walls(mapa, sb_walls)
    return mapa



#print_map(create_world(24, 28, 2, (2, 2, 3, 2)), "▓", "░", "☺", "x", "#")
